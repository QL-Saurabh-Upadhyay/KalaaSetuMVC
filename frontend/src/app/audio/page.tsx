...
import { apiService, EmotionDetectionResponse, SpeechGenerationRequest } from "../services/api";
...
export default function AudioPage() {
  ...
  const handleGenerateAudio = async () => {
    ...
    try {
      ...
      const emotionResponse: EmotionDetectionResponse = await apiService.detectEmotion(text.trim());
      
      if (!emotionResponse.dominant_emotion) {
        throw new Error('Failed to detect emotion from text');
      }

      const detectedEmotion = emotionResponse.dominant_emotion;
  ...
  const getEmotionColor = (emotion: string) => {
    const colors = {
      ...
      sadness: 'text-blue-600 bg-blue-100 dark:text-blue-400 dark:bg-blue-900/20',
      ...
      surprise: 'text-orange-600 bg-orange-100 dark:text-orange-400 dark:bg-orange-900/20',
      disgust: 'text-green-600 bg-green-100 dark:text-green-400 dark:bg-green-900/20',
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-8 w-full max-w-4xl mx-auto mt-8 p-8 bg-white/80 dark:bg-gray-900/80 rounded-3xl shadow-2xl backdrop-blur-md border border-gray-200 dark:border-gray-800">
      <div className="text-center mb-4">
        <h1 className={`text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r ${getGradientClass('primary')} mb-2`}>Text to Speech</h1>
        <p className="text-gray-600 dark:text-gray-300">Convert your text into emotion-aware speech</p>
      </div>
      ...
      <div className="w-full space-y-6">
        ...
        <div className="bg-white/90 dark:bg-gray-800/90 p-6 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
          <h2 className="text-xl font-bold mb-4 text-gray-900 dark:text-gray-100">Text Input</h2>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <button
              ...
              className={`flex-1 rounded-full py-3 px-6 font-bold text-lg shadow-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-green-400 border-none bg-gradient-to-r ${getGradientClass('primary')} text-white hover:scale-105 hover:shadow-2xl disabled:opacity-50 disabled:cursor-not-allowed`}
        ...
        {detectedEmotion && !isGenerating && (
          <div className="bg-white/90 dark:bg-gray-800/90 p-6 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
            <h2 className="text-xl font-bold mb-4 text-gray-900 dark:text-gray-100">Detected Emotion</h2>
            <div className="flex items-center gap-3">
              <span className={`inline-block px-4 py-2 rounded-full text-sm font-medium ${getEmotionColor(detectedEmotion)}`}>
        ...
        {audioUrl && !isGenerating && (
          <div className="bg-white/90 dark:bg-gray-800/90 p-6 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
            <h2 className="text-xl font-bold mb-4 text-gray-900 dark:text-gray-100">Generated Speech</h2>
        ...
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
          <h3 className="font-semibold text-green-800 dark:text-green-200 mb-2">Emotion-Aware Speech Features:</h3>
          <ul className="text-sm text-green-700 dark:text-green-300 space-y-1">
            <li>• Automatically detect emotion from your text</li>
            <li>• Generate speech that matches the detected emotion</li>
            <li>• Support for joy, sadness, anger, fear, surprise, disgust, and neutral</li>
