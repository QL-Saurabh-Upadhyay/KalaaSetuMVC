...
from text_to_video_system import (
    ...
    TextProcessor, AudioGenerator, VisualGenerator, SubtitleGenerator, VideoComposer
)
from flask_api_server import app, generation_jobs
...
class TestTextProcessor:
    ...
    def test_segment_text(self):
        ...
        segments = self.processor.segment_text(text, max_segment_length=50)
        assert len(segments) >= 2
        assert all(len(segment) <= 55 for segment in segments)  # Allow some buffer
    
    def test_extract_key_concepts(self):
        text = "The government announces new digital literacy programs for rural education."
        concepts = self.processor.extract_key_concepts(text)
        assert len(concepts) > 0
        assert any(len(concept) >= 4 for concept in concepts)
...
class TestVideoConfig:
    def test_video_config_creation(self):
        ...
        assert config.domain == Domain.GOVERNANCE
        assert config.environment == Environment.STUDIO
        assert config.duration == 30  # default value
    ...
    def test_video_config_custom_values(self):
        ...
        assert config.include_subtitles == False
...
class TestFlaskAPI:
    ...
    def test_available_options(self, client):
        response = client.get('/api/available-options')
        assert response.status_code == 200
        data = json.loads(response.data)
    ...
    def test_generate_video_endpoint(self, client):
        payload = {
            'text': 'Test video generation with sample text content.',
            'tone': 'formal',
            'domain': 'education', 
            'environment': 'studio',
            'duration': 15
    ...
    def test_generate_video_missing_text(self, client):
        payload = {'tone': 'formal'}
        response = client.post('/api/generate-video',
                              data=json.dumps(payload), 
                              content_type='application/json')
    ...
    def test_job_status_not_found(self, client):
        response = client.get('/api/job-status/nonexistent-job-id')
...
class TestVideoGenerationIntegration:
    ...
    @patch('text_to_video_system.AudioGenerator.generate_narration')
    @patch('text_to_video_system.VisualGenerator.generate_scene_images') 
    @patch('text_to_video_system.VideoComposer.compose_video')
    def test_full_video_generation_pipeline(self, mock_compose, mock_visual, mock_audio, temp_dir):
        # Mock return values
        mock_audio.return_value = os.path.join(temp_dir, 'test_audio.wav')
        mock_visual.return_value = [os.path.join(temp_dir, 'test_image.png')]
        mock_compose.return_value = (
            os.path.join(temp_dir, 'test_video.mp4'),
            Mock(inference_time=30.0, memory_usage=50.0, gpu_usage=25.0,
                 frame_rate=24.0, scene_fidelity_score=0.85, generation_cost=1.5)
        )
        
        # Create dummy files
        with open(mock_audio.return_value, 'w') as f:
            f.write('dummy audio')
        with open(mock_visual.return_value[0], 'w') as f:
            f.write('dummy image')
        with open(mock_compose.return_value[0], 'w') as f:
            f.write('dummy video')
        ...
        video_path, metrics = generator.generate_video("Test text content", config)
        
        assert video_path is not None
        assert metrics.inference_time > 0
        assert 0 <= metrics.scene_fidelity_score <= 1
...
class TestPerformanceBenchmarks:
    def test_text_processing_speed(self):
        ...
        segments = processor.segment_text(long_text)
        end_time = time.time()
    ...
    @patch('text_to_video_system.AudioGenerator.generate_narration')
    def test_audio_generation_timeout(self, mock_audio):
        # Simulate slow audio generation
        def slow_generation(*args, **kwargs):
            time.sleep(0.1)  # Simulate processing time
            return "test_audio.wav"
        
        mock_audio.side_effect = slow_generation
        
        generator = AudioGenerator()
        config = VideoConfig(tone=Tone.FORMAL, domain=Domain.EDUCATION, environment=Environment.STUDIO)
        
        start_time = time.time()
        result = generator.generate_narration("Test text", config)
        end_time = time.time()
...
def run_load_test(num_requests=10, concurrent=False):
    ...
    def make_request():
        try:
            start_time = time.time()
            response = requests.post('http://localhost:5000/api/generate-video', 
                                   json={
                                       'text': 'Load test video generation request.',
                                       'tone': 'formal',
                                       'domain': 'education',
                                       'environment': 'studio',
                                       'duration': 10
...
if __name__ == "__main__":
    ...
    # Run load test if Flask app is running
    try:
        print("\nRunning load test...")
        run_load_test(num_requests=5, concurrent=True)
    except Exception as e:
        print(f"Load test failed (make sure Flask app is running): {e}")

# demo_web_interface.html
"""
<!DOCTYPE html>
<html lang="en">
<head>
